...
namespace ca
{
	namespace math
	{
		...
		template <typename T>
		CA_INLINE vec_t<T, 3> operator* (quat_t<T> const & q, vec_t<T, 3> const & v)
		{
		...
		{
			T hz = euler_xyz.z / 2;// roll
			T hx = euler_xyz.x / 2;// pitch
			T hy = euler_xyz.y / 2;// yaw
		...
		CA_INLINE void set_rotation_by_from_to_direction(quat_t<T> & q, vec3_t<T> const & v0, vec3_t<T> const & v1)
		{
			...
			T s = sqrt(2 + 2 * dot(v0, v1));
			if (s < T(0.01f))//TODO epsilon
			{
				set_identity(q);
			}
			else
