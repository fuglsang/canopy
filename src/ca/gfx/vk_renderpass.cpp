...
namespace ca
{
	namespace gfx
	{
		VkAttachmentLoadOp resolve_loadop(renderloadop loadop)
		{
			switch (loadop)
			{
			case RENDERLOADOP_CLEAR: return VK_ATTACHMENT_LOAD_OP_CLEAR;
			case RENDERLOADOP_DISCARD: return VK_ATTACHMENT_LOAD_OP_DONT_CARE;
			case RENDERLOADOP_LOAD: return VK_ATTACHMENT_LOAD_OP_LOAD;
			default: CA_FATAL("unsupported loadop");
			}
		}
		...
		VkAttachmentStoreOp resolve_storeop(renderstoreop storeop)
		{
			switch (storeop)
			{
			case RENDERSTOREOP_DISCARD: return VK_ATTACHMENT_STORE_OP_DONT_CARE;
			case RENDERSTOREOP_STORE: return VK_ATTACHMENT_STORE_OP_STORE;
			default: CA_FATAL("unsupported storeop");
			}
		}
		...
		void create_renderpass(renderpass_t * renderpass, device_t * device, renderattachment_t * attachments, u32 attachment_count)
		{
			...
			//TODO tls stackarena
			VkAttachmentDescription * attachment_descs = mem::arena_alloc<VkAttachmentDescription>(device->arena, attachment_count);
			VkAttachmentReference * attachment_refs = mem::arena_alloc<VkAttachmentReference>(device->arena, attachment_count);
			VkImageView * attachment_views = mem::arena_alloc<VkImageView>(device->arena, attachment_count);
			...
			mem::arena_free(device->arena, attachment_descs);
			mem::arena_free(device->arena, attachment_refs);
			mem::arena_free(device->arena, attachment_views);
